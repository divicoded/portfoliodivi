    ...
    Promise.all(
      this.items.map(
        item =>
          new Promise<HTMLImageElement | null>(resolve => {
            ...
            img.onerror = () => {
              console.warn(`Failed to load texture: ${item.image}`);
              resolve(null);
            };
            img.src = item.image;
          })
      ...
      images.forEach((img, i) => {
        ...
        try {
            ...
            console.warn('Error drawing image to canvas:', e);
        }
      });
...
const defaultItems: MenuItem[] = [
  {
    ...
    title: 'Void',
    description: 'Empty state visualization'
  }
];
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [], scale = 1.0 }) => {
  ...
  // State is kept for logic, but UI is removed as requested
  ...
  useEffect(() => {
    ...
    if (canvas) {
      try {
        ...
        console.error("Failed to initialize InfiniteGridMenu", e);
      }
    }
